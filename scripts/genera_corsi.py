...
(function(){
  ...
  // Il salto all'ancora lo fa il browser su una stima: le schede portano
  // content-visibility:auto con 108px supposti, e il salto si calcola su
  // quelli. Poi si disegnano le schede vicine all'arrivo, che a 1280px sono
  // alte 105, e il bersaglio sale di 3px per ognuna - misurato il 23/09/2026:
  // con 88 schede #r-allinea-mente atterrava a 90px sotto un tetto di 136
  // (senza content-visibility: 148, esatto). Colpisce solo quello che sta
  // DOPO l'elenco (le realta'), e cresce con le schede: e' per questo che e'
  ...
  else window.addEventListener('load',riallinea);
  window.addEventListener('hashchange',riallinea);
  if(!bar) return;
