...
# sono dati e non logica, e perche' index.html li ripeteva a mano in ogni card.
SVG_IG = ('<svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">'
          ...
          '-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 '
          '1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 '
          '2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668'
          '-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689'
          '.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78'
          '-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 '
          '2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0'
          '-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0'
          '-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c'
          '-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 '
          '1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>')

SVG_FB = ('<svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">'
          ...
          '1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c'
          '-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C'
          '19.612 23.027 24 18.062 24 12.073z"/></svg>')


def blocco_social_home():
    """Le card per provincia della sezione "Seguici" sulla home.

    PERCHE' E' GENERATA. Era il secondo elenco di province scritto a mano su
    index.html, e ha fatto lo stesso guasto del footer: l'08/09/2026 le card
    erano due — Alessandria e Asti — con Cuneo aperta dal 04/08 e presente in
    PROVINCE_IG col suo curatore accanto. Il footer di quella stessa pagina la
    nominava; questa sezione no. Non lo prendeva nessuna prova, perche' non era
    un file rimasto indietro: era un pezzo che nessuno aveva il compito di
    aggiornare. Adesso la quarta provincia entra da se' con PROVINCE_PUBBLICATE.

    Facebook si stampa solo dove c'e' (Cuneo non ce l'ha): quello che manca non
    si inventa, e' la regola di link_luoghi(). Nessuna icona accanto al nome
...
def aggiorna_nav():
    ...
    def blocco(assoluto=False, inline=False):
        return {
            ...
            # stanno nel suo <style>, quindi non serve vestirle.
            'HOME-SOCIAL': blocco_social_home(),
        }
...
def blocco_ecosistema(qui=None):
    ...
    # Sulla home la sezione ha l'occhiello e la riga di sotto come tutte le
    # altre fasce di quella pagina. Non e' decorazione: senza, un <h2> di
    # 1,5rem in mezzo a titoli Playfair da 38px si legge come una didascalia
    # capitata li' per sbaglio (Patrick, 08/09/2026). In coda a una pagina
